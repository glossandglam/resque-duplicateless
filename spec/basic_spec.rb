...
describe ResqueUniqueJob do
  describe "Basic" do
    ...
    it "can add a job to the queue" do
      ...
      expect(Resque.size("queue_1")).to eql(1)
    end
    ...
    it "can enqueue two of the same job, to the same queue, if the params are different" do
      ...
      expect(Resque.size("queue_1")).to eql(2)
    end
    ...
    it "can enqueue the same job with the same params to two different queues" do
      ...
      expect(Resque.size("queue_1")).to eql(1)
      expect(Resque.size("queue_2")).to eql(1)
    end
    ...
    it "will maintain the job order when trying to uniquely add" do
      ...
      expect(Resque.size("queue_1")).to eql(5)
      args = Resque.data_store.everything_in_queue("queue_1").map{|s| JSON.parse(s)["args"].first}
